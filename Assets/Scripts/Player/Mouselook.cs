...
public class Mouselook : MonoBehaviour
{
	...
	void Update()
	{
        ...
        //Debug.Log("Joy x: "  + Input.GetAxis("Joy X") + " || " + "Joy Y: " + Input.GetAxis("Joy Y"));
        if (Input.GetAxis("Joy X") > 0.3f || Input.GetAxis("Joy X") < -0.3f || Input.GetAxis("Joy Y") > 0.3f || Input.GetAxis("Joy Y") < -0.3f)
	    {
	        mouseDelta = new Vector2(Input.GetAxisRaw("Joy X"), Input.GetAxisRaw("Joy Y"));
	    }
	    else
	    {
